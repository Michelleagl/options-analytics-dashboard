...
st.set_page_config(page_title="Greeks · Options Analytics Dashboard", page_icon="📊", layout="wide")
inject_css()
...
def sweep_greeks_vs_strike(strikes_tuple, S0, r, q, tau, sigma_atm, heston_params_tuple, option_type):
...
@st.cache_data(ttl=1800, show_spinner=False)
def sweep_greeks_vs_spot(spot_tuple, K, r, q, tau, sigma_atm, heston_params_tuple, option_type):
    v0_, theta_, kappa_, xi_, rho_ = heston_params_tuple
    rows = []
...
with st.spinner("Calculando Greeks a través de strikes y de spot (bump-and-reprice)..."):
    df_strike = sweep_greeks_vs_strike(tuple(strikes_for_sweep), ctx.S0, ctx.r, ctx.q, ctx.tau, sigma_atm, tuple(heston_params), ctx.option_type)
    df_spot = sweep_greeks_vs_spot(tuple(spot_range), ctx.strike, ctx.r, ctx.q, ctx.tau, sigma_atm, tuple(heston_params), ctx.option_type)
